...
試行錯誤、紆余曲折、侃々諤々、などなど考えることが絶えなかったですが、最終的にこのビジュアルに落ち着きました。
（というより、切羽詰まってこれに落ち着いた感じはありますが…）


TECH☆TECHというサークル自体、興味や得意な分野が個々人で割とばらけているので、集団としての位置づけが不明瞭になってしまいがちでした。
しかし、これを逆手に取れば、メンバーがそれぞれで作ったり学んだりしたそのすべてを総体としてTECH☆TECHとも言えます。  
...
# 実装の工夫
実装にはp5.jsというライブラリを使っています。
私が普段から愛用しているクリエイティブコーディングのツールで、比較的簡単に扱えますし、雑に書いても動くので気に入っています。
...
円自体もそうですが、移動中に分離するものも含めてに入れるようにしています。
極端に線が短いやつは描画されないようにしました。

## 重複の回避
サークルパッキング的な手法ですが、円同士が移動後に重ならないようにしています。
だって、その方が見栄えがいいし。
...
ライセンスは[NYSL（煮るなり焼くなり好きにしろライセンス）](https://www.kmonos.net/nysl/)です。
Forkしたときは教えてやると僕が喜びます。
